...
class Point(object):
    ...
    def __init__(self, x, y):
        ...
        self.y = y
        self.count = 0
    # Both init and repr start with double underscores.
    # First one starts when someone constructs a point.
    # This will feed back the current position of that point.

    def __repr__(self):
        return'Point({},{})'.format(self.x, self.y)
    # Returns a string that represents a Point like this: 'Point(x, y)' where x and y
    # are replaced by this Point's x and y coordinates.
    ...
    # Changes the instance variables x and y that store the position of this Point to
    # the given x and y. This has the effect of "moving" this Point TO the given (x, y).

    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.count = self.count + 1
    # Adds the dx and dy to the instance variable x and y. This stores the position
